...
\Hrule
  ...
  \begin{center}
    ...
    \begin{tabular}[t]{l|r|l|l}
      ...
      & \texttt{G + A - GOT} / \texttt{G + A}$^\dagger$ \\
      \texttt{Reserved} & 44 & &  \\
      \texttt{R_386_SEG16} & 45 & \textit{word16} & \texttt{A + (S >> 4)} \\
      \texttt{R_386_SUB16} & 46 & \textit{word16} & \texttt{A - S} \\
      \texttt{R_386_SUB32} & 47 & \textit{word32} & \texttt{A - S} \\
     \cline{1-4}
...
\texttt{R_386_GOT32} and \texttt{R_386_GOT32X} relocations can refer
to GOT address, which is a memory operand, or GOT index, which is an
immediate operand:

\begin{description}
\item[\code{name@GOT}] It refers to the address of the symbol's global
...
\item[\code{\$name@GOT}] It refers to the index of the symbol's global
offset table entry
...
\texttt{R_386_SEG16}, \texttt{R_386_SUB16} and \texttt{R_386_SUB32}
relocations should only be used in 16-bit real mode segmented code.
Linker should ignore overflow of \texttt{R_386_16} and \texttt{R_386_SUB16}
relocations when a pair of consecutive \texttt{R_386_16} and
\texttt{R_386_SUB16} relocations are applied at the same offset.
Multiple relocations at the same offset are cumulative.  The previous
relocation result becomes the addend for the current relocation.
