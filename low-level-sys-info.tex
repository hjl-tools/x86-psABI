...
(\reg{bnd0} - \reg{bnd3}).  For purposes of parameter passing and function return,
the lower 64 bits of \reg{bndN} specify lower bound of the corresponding
parameter, and the upper 64 bits specify upper bound of the parameter.
The upper bound is represented in one's complement form.
...
\item If the size of an object is larger than eight \eightbytes, or
  ...
  can be passed in a registeri, otherwise, it will be passed in memory.}.
...
\item If the class is INTEGER or POINTER, the next available register
  of the sequence \RDI, \RSI, \RDX, \RCX, \reg{r8} and \reg{r9} is
...
\item If the class is POINTER and the eightbyte is passed in a general
  ...
  For each AMD64 call there may be up to two such pointer arguments,
  ...
  For each X32 call there may be up to eight such pointer arguments and the bounds
  are associated with (\emph{<return address stack location>--8}),
  (\emph{<return address stack location>--12}), ...,
  (\emph{<return address stack location>--36}).
...
The corresponding bound registers allocation is given in Figure \ref{fig_bounds_allocation_example},
