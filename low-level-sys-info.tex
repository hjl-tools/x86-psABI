...
\subsubsection{Empty Record}

Empty record is defined as
\begin{enumerate}
\item Collection is a structure, union or C++ class.
\item Empty collection is one of
  \begin{itemize}
  \item A collection without member.
  \item A collection with only empty collections.
  \item An array of empty collections.
  \end{itemize}
\item Empty record is Plain Old Data (POD) for the purpose of layout
      and one of
  \begin{itemize}
  \item A collection without member.
  \item A collection with only empty collections.
...
No memory slot nor register should be used to pass or return an object
of empty record.
...
rules in section~\ref{data_representation}.
...
The \textindex{auxiliary vector}\footnote{The \textindex{auxiliary
