...
When an argument of
type \code{_Bool} is passed in a register or on the stack, bit 0
contains the truth value and bits 1 to 31 shall be zero.

When a value of type signed/unsigned \code{char} or \code{short} is
returned in a register, bits 0 to 7 for \code{char} and bits 0 to 15
for \code{short} contain the value and other bits are left unspecified.
When an argument of signed/unsigned type \code{char} or \code{short} is
passed in a register or on the stack, it shall be sign/zero extended to
signed/unsigned \code{int}.
