...
\hline
\reg{xmm0}, \reg{ymm0}, & scratch registers; also used to pass and
return \\
\reg{zmm0} & \code{__m128}, \code{__m256}, \code{__m512} parameters & No\\
\hline
\reg{xmm1}--\reg{xmm2},& scratch registers; also used to pass
\code{__m128}, & \\
\reg{ymm1}--\reg{ymm2}, & \code{__m256}, \code{__m512} parameters & No \\
\reg{zmm1}--\reg{zmm2} &  & \\
\hline
\reg{xmm3}--\reg{xmm7},& scratch registers & \\
\reg{ymm3}--\reg{ymm7},& & No \\
\reg{zmm3}--\reg{zmm7} & & \\
\hline
...
\phantom{extern structparm func (}__m512 x, __m128 y,\\
\phantom{extern structparm func (}__m256 z, __m256 t);\\
\\
func (i, v, s, w, x, y, z,  t);\\
\cline{1-1}
\end{tabular}
}
\end{center}
...
\code{i}             & 4 bytes  &\\
\cline{1-2}
Return value pointer & 4 bytes  & $\longleftarrow$ \ESP (64-byte aligned)\\
\cline{1-2}
\end{tabular}
\end{center}
\Hrule
...
passed on the stack, including \texttt{__m64}, \texttt{__m128},
\texttt{__m256} and \texttt{__m512}.
This rule applies to both named and unnamed parameters.
