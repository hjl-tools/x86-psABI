...
\reg{xmm7}). For purposes of parameter passing and function return,
...
alignment according to the values in Figure~\ref{basic-types}.  There is
an exception for \texttt{_Decimal64}, which is treated as having an
...
\begin{itemize}
  \item The first three parameters of type \texttt{__m64} are passed in
	\reg{mm0}, \reg{mm1}, and \reg{mm1}.
  \item The first three parameters of type \texttt{__m128} are passed in
	\reg{xmm0}, \reg{xmm1}, and \reg{xmm1}.\footnote{The SSE and AVX
	registers share resources. Therefore, if the first \texttt{__m128}
	parameter gets assigned to \reg{xmm0} , the first \texttt{__m256}
	parameter after that is assigned to \reg{ymm1} and not
	\reg{ymm0}.}
\end{itemize}
...
Figure~\ref{fig_returning_locations} lists the location used to return a
...
frame layout given in Figure~\ref{fig_stack_frame_layout} shows the
frame before calling the function.
