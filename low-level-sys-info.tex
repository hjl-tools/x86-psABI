...
are passed in \reg{eax}, \reg{edx}, and \reg{ecx}\footnote{If
  \reg{eax} is used to return a value in memory, only \reg{edx} and
  \reg{ecx} are available to pass parameters.}\footnote{A parameter is
...
\begin{itemize}
  \item Scalar-class aggregate types no larger than 8 bytes are returned
