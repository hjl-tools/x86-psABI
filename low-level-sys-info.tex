...
The 128-byte area beyond the location pointed to by \ESP is considered
...
\EBX & callee-saved register; used to hold the GOT pointer when
...
\reg{xmm1}--\reg{xmm2}, \reg{ymm1}--\reg{ymm2} &  scratch registers;
also used to pass \code{__m128}, \code{__m256} parameters & No\\
\reg{xmm3}--\reg{xmm7}, \reg{ymm3}--\reg{ymm7} & scratch registers & No\\
...
\footnote{Note that the rest of \EAX is undefined, only the contents
...
\hline
\EDI:    &\code{e}      &\reg{xmm0}:&\code{s.d}&\code{0:} &\code{ld} \\
\ESI:    &\code{f}      &\reg{xmm1}:&\code{m}  &\code{16:}&\code{j} \\
\EDX:    &\code{s.a,s.b}&\reg{ymm2}:&\code{y}  &\code{24:}&\code{k} \\
\ECX:    &\code{g}      &\reg{xmm3}:&\code{n}  &          & \\
\end{tabular}
