...
and \code{__float128} types are optional.
...
\paragraph{Scalar-class Aggregates}

Scalar-class aggregate types (\texttt{structs} and \texttt{unions}) are
aggregate types no larger than 4 bytes, whose fields are scalars or
other scalar-class aggregate types.
...
The first three parameters of scalar types no larger than 4 bytes or
scalar-class aggregate types (\texttt{structs} and \texttt{unions})
are passed in \reg{eax}, \reg{edx}, and \reg{ecx}.  The rest of parameters
...
functions; also used to return 32-bit scalar and scalar-class
...
\begin{itemize}
  \item Scalar-class aggregate types are returned in \reg{eax}.
