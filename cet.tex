...
\begin{enumerate}
  \item When loading an executable without interpreter, enable and lock
    IBT if \code{GNU_PROPERTY_X86_FEATURE_1_IBT} is set on the executable.
  \item When loading an executable with an interpreter, enable IBT if
    ...
    otherwise lock IBT.
 \item
   \begin{sloppypar} After IBT is enabled, when loading a shared object
...
\begin{enumerate}
  \item When loading an executable without interpreter, enable and lock
    ...
    executable.
  \item When loading an executable with an interpreter, enable SHSTK if
