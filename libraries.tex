...
\code{
\begin{tabular}{l}
    uint32 \_Unwind\_GetGR\\
    \ \ (struct \_Unwind\_Context *context, int index);\\
\end{tabular}
}

This function returns the 32-bit value of the given general register.
The register is identified by its index as given in
table~\ref{tbl-reg-num-map}.
...
This function sets the 32-bit value of the given register, identified by
...
\code{
\begin{tabular}{l}
    uint32 \_Unwind\_GetLanguageSpecificData\\
            (struct \_Unwind\_Context *context);\\
\end{tabular}
}
...
\code{
\begin{tabular}{l}
    uint32 \_Unwind\_GetRegionStart\\
    \ \ (struct \_Unwind\_Context *context);\\
\end{tabular}
}
