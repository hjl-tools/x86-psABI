...
If there are \texttt{R_X86_64_PC32_BND} or \texttt{R_X86_64_PLT32_BND}
relocations in input, linker should switch to the BND procedure
linkage table (see figure \ref{bnd_small_med_plt}) together with an
additioal procedure linkage table (see figure \ref{ext_small_med_plt})
to preserve bound registers for symbol lookup in branch instructions
with the \texttt{BND} (\code{0xf2}) prefix.  The \texttt{BND}
(\code{0xf2}) prefix is optional in procedure linkage table entries
for symbol lookup in branch instructions without the \texttt{BND}
(\code{0xf2}) prefix since bound registers will be cleared when branch
to the procedure linkage table entry is taken.
...
\begin{verbatim}
  .APLT1: bnd jmp  name1@GOTPCREL(%rip)
          nop
  .APLT2: jmp      name2@GOTPCREL(%rip)       # 8 bytes from .APLT1
          xchg     %ax,%ax
...
\item The first instruction jumps to the address in the global offset
  ...
  real address of \code{name1}.  When the BND procedure linkage table
